...
class PERIPHERAL extends BOARDOBJECT
    ...
    getWidth: => @size.width*@size.scale
    getHeight: (inputpincount, outputpincount) => @size.space * (4+(3*(math.max(inputpincount, outputpincount)-1))) * (@size.scale/7)
    ...
    draw: =>
        -- dropshadow
        love.graphics.setColor(graphicSettings.gateDropshadowColor)
        height = @getHeight(@inputpincount, @outputpincount)
        love.graphics.rectangle("fill", @pos.x + graphicSettings.gateDropshadowOffset, @pos.y + graphicSettings.gateDropshadowOffset, @getWidth(), height, graphicSettings.gateDropshadowRounding)
    ...
    isInside: (x,y) =>
        x >= @pos.x and x <= @pos.x + @getWidth() and y >= @pos.y and y <= @pos.y + @getHeight(@inputpincount, @outputpincount)

    getInputPinAt: (x,y) =>
        return @inputpin if lume.distance(x, y, @pos.x, @pos.y + (@getHeight(@inputpincount, @outputpincount)/2)) < 10

    getOutputPinAt: (x,y) =>
        return @outputpin if lume.distance(x, y, @pos.x + @getWidth(), @pos.y + (@getHeight(@inputpincount, @outputpincount)/2)) < 10
...
class INPUT extends PERIPHERAL
    ...
    drawMe: =>
        ...
        love.graphics.rectangle("fill", @pos.x+pad + (@state and @getWidth()/2-pad or 0), @pos.y+pad, @getWidth()/2-pad, @getHeight(@inputpincount, @outputpincount)-pad*2)
...
class OUTPUT extends PERIPHERAL
    ...
    drawMe: =>
        love.graphics.setColor(1, 1, 1)
        printCentered("OUT #{@id-2000}", @pos.x + (@getWidth()/2), @pos.y + (@getHeight(@inputpincount, @outputpincount)/2))
