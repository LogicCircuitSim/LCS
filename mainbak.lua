...
function love.load()
	...
	menuTransform = love.math.newTransform(love.graphics.getWidth(),0,0,1)
...
function love.update(dt)
	...
	menuTargetX = love.graphics.getWidth() * (currentMenu - 1)
	if menuX ~= menuTargetX then
		menuX = lume.lerp(menuX, menuTargetX, 0.1)
	end
...
function love.draw()
	...
	if shouldShowMenu(currentMenu, menus.title) then
		love.graphics.print('L.C.S.', bigfont, love.graphics.getWidth()/2, 100)
	elseif shouldShowMenu(currentMenu, menus.list) then
	...
	elseif shouldShowMenu(currentMenu, menus.board) then
		-- camera
		mx, my = camera:getScreenPos(love.mouse.getPosition())
		camera:set()
		-- grid points
	...
	elseif shouldShowMenu(currentMenu, menus.settings) then
		-- do settings stuff
	elseif shouldShowMenu(currentMenu, menus.about) then
		-- do about stuff
	end
	love.graphics.pop()
	...
	love.graphics.print(('Menu: %d | MenuX: %d | MenuTargetX: %d'):format(currentMenu, menuX, menuTargetX), 600, 30)
end
...
function love.keypressed(key, scancode, isrepeat)
	...
	if keyPressed(key, 'escape') then
		if currentMenu>1 then currentMenu=currentMenu-1
		elseif currentMenu<1 then currentMenu=currentMenu+1
		end
	end
...
function shouldShowMenu(curmenu, menu)
	return menu==curmenu or ((menu==curmenu-1 or menu==curmenu+1) and menuIsSliding == true)
end
