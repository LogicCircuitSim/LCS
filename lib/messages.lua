...
messages.draw = function(debug)
    ...
    local y = messages.y
    lume.each(messages.list, function(msg)
        love.graphics.draw(msg.text, x, y)
        if debug then love.graphics.print(msg.timeout, x+msg.text:getWidth()+10, y) end
...
messages.addMessage = function(msg, wraplimit, align)
    ...
    if wraplimit then
        text:setf(msg, wraplimit, align or love.graphics.AlignMode.left or 0)
    end
    local message = {}
    message.text = text
